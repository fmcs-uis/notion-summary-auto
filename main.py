...
def get_existing_materials():
    ...
    while has_more:
        ...
        for page in data["results"]:
            try:
                material = page["properties"]["材料編號"]["title"][0]["plain_text"]
...
def write_to_target_db(summary):
    ...
    for material, info in summary.items():
        ...
        data = {
            "properties": {
                "材料編號": {"title": [{"text": {"content": material}}]},
                "數量": {"number": total},
                "位置": {"multi_select": [{"name": location}] if location else []},
                "姓名": {"rich_text": [{"text": {"content": name}}] if name else []}
            }
        }
        ...
        if material in existing:
            ...
            print(f"✅ 更新 {material} | 狀態碼: {response.status_code}")
        else:
            data["parent"] = {"database_id": TARGET_DB_ID}
            url = "https://api.notion.com/v1/pages"
            response = requests.post(url, headers=HEADERS, json=data)
            print(f"➕ 新增 {material} | 狀態碼: {response.status_code}")
